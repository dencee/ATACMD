...
//------------------------------------------------------------------------------
// Description: x86 Interrupt jump table: http://www.ctyme.com/intr/int.htm
//
// Input:  
//
// Output: 
//
//------------------------------------------------------------------------------
unsigned int GetPciByte( unsigned int busNum, unsigned int devNum, unsigned int funNum, unsigned int regNum )
{
   union REGS ir, or;

   ir.x.ax = PCI_READ_CONFIGURATION_BYTE;
   ir.x.bx = ( ( busNum << 8 ) | ( devNum << 3 ) | funNum );
   ir.x.di = regNum;
   int86( X86_INTERRUPT_1A, &ir, &or );
   if ( or.x.cflag )
   {
      return 0xFFFF;
   }
...
//
//------------------------------------------------------------------------------
unsigned int GetPciWord( unsigned int busNum, unsigned int devNum, unsigned int funNum, unsigned int regNum )
{
   union REGS ir, or;

   // Register needs to be a multiple of 2
   if ( regNum & 0x0001 )
   {
      return 0xFFFF;
   }
   
   ir.x.ax = PCI_READ_CONFIGURATION_WORD;
   ir.x.bx = ( ( busNum << 8 ) | ( devNum << 3 ) | funNum );
   ir.x.di = regNum;
   int86( X86_INTERRUPT_1A, &ir, &or );
   if ( or.x.cflag )
...
//
//------------------------------------------------------------------------------
unsigned int GetPciClassCode( unsigned int busNum, unsigned int devNum, unsigned int funNum )
{
   return ( GetPciByte( busNum, devNum, funNum, offsetof( PCIRegisters_t, classCode ) ) );
}
...
//
//------------------------------------------------------------------------------
unsigned int GetPciSubClassCode( unsigned int busNum, unsigned int devNum, unsigned int funNum )
{
   return ( GetPciByte( busNum, devNum, funNum, offsetof( PCIRegisters_t, subClass ) ) );
}
...

{
   ...
   _OUTPW( pio_bmide_base_addr + BM_PRD_ADDR_LOW,
            (unsigned int) ( temp & 0x0000ffffL ) );
   _OUTPW( pio_bmide_base_addr + BM_PRD_ADDR_HIGH,
